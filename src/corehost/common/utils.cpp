...
bool is_exe_enabled_for_execution(const pal::string_t& own_path)
{
    ...
    if (binding.compare(0, hi_len, &hi_part[0]) == 0 &&
        binding.compare(hi_len, lo_len, &lo_part[0]) == 0)
    {
        trace::error(_X("The binding resource string was not replaced. The current value is: %s"), binding.c_str());
        return false;
    }
    ...
    return (pal::cstrcasecmp(utf8_dll_filename.data(), binding.c_str()) == 0);
}
#endif
